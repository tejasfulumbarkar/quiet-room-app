...
import { getLevelInfo, getLevelFromXP } from "@/lib/leveling-system"
...
export default function DashboardPage() {
  ...
  // Show small onboarding prompt after philosophy gate closes if user has no tasks yet
  const [showOnboardingPrompt, setShowOnboardingPrompt] = useState(false)
  const [isCreatingFromOnboarding, setIsCreatingFromOnboarding] = useState(false)
  const [onboardingHighlight, setOnboardingHighlight] = useState<null | "tasks" | "goals" | "zen">(null)

  const handleClosePhilosophyGateAndMaybeShowOnboard = async () => {
    if (user) {
      await supabase.from("profiles").update({ has_seen_philosophy_gate: true }).eq("user_id", user.id)

      // Check if user has any tasks; if none, show onboarding prompt
      const { count } = await supabase.from("tasks").select("id", { count: "exact", head: true }).eq("user_id", user.id)
      if (!count || count === 0) {
  ...
  const handleOnboardingChoice = (choice: "tasks" | "goals" | "zen") => {
    if (choice === "tasks") {
      setIsCreatingFromOnboarding(true)
      setCurrentPage("tasks")
      setOnboardingHighlight("tasks")
    } else if (choice === "goals") {
      setCurrentPage("goals")
      setOnboardingHighlight("goals")
    } else if (choice === "zen") {
      setCurrentPage("zen-mode")
      setOnboardingHighlight("zen")
    }
  ...
  const fetchActivityData = async () => {
    ...
    // Fetch XP entries from activity_log (includes tasks, zen sessions, rewards, etc.) for the last 365 days
    ...
    if (activities) {
      // Group by date and sum XP from all activity types
      const activityMap = new Map<string, number>()

      activities.forEach((a: any) => {
        const ts = a.created_at || a.created_at
        if (ts) {
          const date = new Date(ts).toISOString().split("T")[0]
          const currentXP = activityMap.get(date) || 0
          activityMap.set(date, currentXP + (a.xp_earned || 0))
        }
      })
  ...
  const handleCreateTask = async (newTask: any) => {
    ...
    const xpForCreation = isCreatingFromOnboarding ? 5 : 3
    ...
    if (!error && inserted) {
      // If this was created via onboarding, award immediate XP and log activity
      if (isCreatingFromOnboarding) {
        try {
          const { data: profile } = await supabase.from("profiles").select("total_xp, current_xp, level").eq("user_id", user.id).maybeSingle()

          const prevTotal = profile?.total_xp || 0
          const prevCurrent = profile?.current_xp || 0
          const prevLevel = profile?.level || 1

          const newTotal = prevTotal + xpForCreation
          const newCurrent = prevCurrent + xpForCreation
          const newLevel = getLevelFromXP(newTotal)

          await supabase.from("profiles").update({ total_xp: newTotal, current_xp: newCurrent, level: newLevel }).eq("user_id", user.id)

          await supabase.from("activity_log").insert({
            user_id: user.id,
            activity_type: "task_creation",
            related_id: inserted.id,
            xp_earned: xpForCreation,
            metadata: { note: "First task bonus" },
          })

          setXpToastData({ xp: xpForCreation, message: "+5 XP for creating your first task" })
          setShowXPToast(true)
          ...
          console.error("Error awarding first-task XP:", e)
        }
        setIsCreatingFromOnboarding(false)
      }
  ...
  const toggleComplete = async (id: string, currentState: boolean) => {
    ...
    if (!error && newState) {
      ...
      if (profile) {
        ...
        console.log(" Task completed - Profile XP:", profile.current_xp, "/", profile.xp_to_next_level)
      }
              ...
              <div className="flex-1">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">One honest focus session at a time.</p>
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="dark">
      ...
      {showPhilosophyGate && <PhilosophyGate onClose={handleClosePhilosophyGateAndMaybeShowOnboard} />}
      {showOnboardingPrompt && (
        <OnboardingModal isOpen={showOnboardingPrompt} onClose={() => setShowOnboardingPrompt(false)} onChoose={handleOnboardingChoice} />
      )}
      ...
      <div className="flex h-screen bg-background text-foreground">
        ...
        {onboardingHighlight === "tasks" && <OnboardingHighlight label="new task" onDismiss={() => setOnboardingHighlight(null)} />}
        {onboardingHighlight === "goals" && <OnboardingHighlight label="new goal" onDismiss={() => setOnboardingHighlight(null)} />}
        {onboardingHighlight === "zen" && <OnboardingHighlight label="start focus" onDismiss={() => setOnboardingHighlight(null)} />}
