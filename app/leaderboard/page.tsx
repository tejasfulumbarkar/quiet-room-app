...
export default function LeaderboardPage() {
  ...
  const handleClaimAura = async (entry: LeaderboardEntry) => {
    ...
    try {
      // Read current aura, then add the claimable amount (avoid overwrite)
      ...
      const currentAura = (profileData?.aura as number) || 0
      const newAura = currentAura + entry.claimable_aura
      ...
      // Log the aura claim in activity_log for auditing
      try {
        await supabase.from("activity_log").insert({
          user_id: entry.user_id,
          type: "aura_claim",
          amount: entry.claimable_aura,
          metadata: null,
          created_at: new Date().toISOString(),
        })
      } catch (logErr) {
        console.warn("Failed to log aura claim:", logErr)
      }
